...
const settingsWidgets = new GObject.Class({
    ...
    _init() {
        ...
        label = new Gtk.Label({ label: 'Icon button size', hexpand: true, halign: Gtk.Align.START });
        widget = new Gtk.SpinButton({halign:Gtk.Align.END});
        widget.set_range(0, 24);
		widget.set_increments(1, 1);
		this._settings.bind('icon-button-size', widget, 'value', Gio.SettingsBindFlags.DEFAULT);
        ...
        label = new Gtk.Label({ label: 'Tray icons left margin', hexpand: true, halign: Gtk.Align.START });
        widget = new Gtk.SpinButton({halign:Gtk.Align.END});
        widget.set_range(0, 24);
		widget.set_increments(1, 1);
		this._settings.bind('tray-left-margin', widget, 'value', Gio.SettingsBindFlags.DEFAULT);
